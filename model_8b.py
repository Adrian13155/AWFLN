...
class AWFLN(nn.Module):
    def __init__(self, scale_factor):
        ...
        self.blk_8_30_3 = nn.Sequential(
            # nn.Conv2d(2, 64, kernel_size=9, padding=4),
            nn.Conv2d(8, 30, kernel_size=3, padding=1),
            nn.PReLU()
        )
        ...
        self.up_sample2 = UpsampleBLock(8, 2)  ### for 4 band 4, for 8 band 8
        ...
        self.lu_block2 = RMRS(30)  # need to be here

    #######################
    def forward(self, ms_up, ms_org, pan):
        data_cat = torch.cat([ms_up, pan], dim=1)  # need to change

        ms_org_up = self.up_sample4(ms_org)  ## in_channels, in_channels * up_scale ** 2

        data1 = torch.cat([ms_org_up, pan], dim=1)
        pan_conv = self.blk_9_30_3(data1)
        ms_up_conv = self.blk_8_30_3(ms_up)

        out1 = torch.cat([ms_up_conv, pan_conv], dim=1)  # need to be here

        out2 = self.lu_block1(out1)

        out3 = self.blk2(out2)
        out3 = self.lu_block2(out3)  # should be here

        out8 = self.conv6(out3)  # need change
...
class RMRS(nn.Module):
    ...
    def forward(self, x):
        out1 = x
        out21 = self.conv2_1(out1)  # need to be here
        out22 = self.conv2_2(out1)  # need to be here
        out23 = self.conv2_3(out1)  # need to be here

        out2 = torch.cat([out21, out22, out23], 1)  # need to be here
        # out2 = self.eca(out2)
        # out2 = torch.cat([out31, out32, out33], 1)
        out2 = self.conv3_1(out2)  # need to be here
        out2 = self.conv3_2(out2)  # need to be here
