...
def main(opt):
    ...
    optimizer_G = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.9,0.999))
    ...
    for epoch in range(opt.epoch_start,num_epoch):
        ...
        for index, data in enumerate(pbar):
            ...
            restored  = model(inp_lms, inp_ms, inp_pan)
        ...
        if epoch % 10== 0:
            ...
            with torch.no_grad():
                ...
                for index, datas in enumerate(tqdm(val_dataloader)):
                    ...
                    output = model(inp_lms, inp_ms, inp_pan)
            ...
            ## record
            logger.info('Epoch:[{}]\t PSNR_{} = {:.4f}\t BEST_PSNR = {:.4f}\t BEST_epoch = {}'.format(
                        epoch, opt.satellite, psnr, best_psnr, best_index))
            print('Epoch:[{}]\t PSNR_{} = {:.4f}\t BEST_PSNR = {:.4f}\t BEST_epoch = {}'.format(
                        epoch, opt.satellite, psnr, best_psnr, best_index))
...
def get_opt():
    ...
    parser.add_argument('--exp_name', type=str, default='LAGConvForWV2', help='experiment name')
    parser.add_argument('-learning_rate', help='Set the learning rate', default=0.0005, type=float)
    parser.add_argument('-batch_size', help='批量大小', default=16, type=int)
    ...
    parser.add_argument('-satellite', help='GF1/QB/WV2/WV4', default='WV2', type=str)
    parser.add_argument('-save_dir', help='日志保存路径', default='/PublicData/xmm/project/AWFLN/experiment', type=str)
    parser.add_argument('-gpu_id', help='gpu下标', default=1, type=int)
